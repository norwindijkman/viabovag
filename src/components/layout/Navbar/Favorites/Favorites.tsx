  ...
  return (
    <div>
      <div className="tw-relative">
        <button onClick={() => setSidePanelIsOpen(!sidePanelIsOpen)} className="tw-w-10 tw-h-10 tw-justify-items-center">
        ...
        {favoritesCount ? (
          <div className="tw-absolute tw-select-none tw-top-0 tw-right-0 tw-bg-[#E74C3C] tw-w-4 tw-h-4 tw-text-[.75rem] tw-justify-items-center tw-text-center tw-text-white tw-rounded-full">
            {favoritesCount}
          </div>
        ) : ''}
      </div>
      {sidePanelIsOpen ? <SidePanel handleClose={() => setSidePanelIsOpen(false)} /> : ''}
    </div>
  );
}
